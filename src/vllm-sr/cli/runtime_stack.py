...
RUN_RESOURCE_LABEL = "com.vllm.semantic-router.run"
...
class RuntimeStackLayout:
    ...
    def ownership_labels(self) -> tuple[tuple[str, str], ...]:
        ...
        if self.run_id is not None:
            labels.append((RUN_RESOURCE_LABEL, self.run_id))
        return tuple(labels)
...
def resolve_runtime_stack(
    *,
    ...
    resolved_run_id = normalize_run_id(
        run_id if run_id is not None else os.getenv(RUN_ID_ENV)
    )
...
def normalize_run_id(raw_value: str | None) -> str | None:
    """Return an explicit run generation, or ``None`` for legacy stack mode.

    A run id is deliberately not generated by the CLI: a separately invoked
    ``stop`` must receive the same id that ``serve`` used. Integration entry
    points own generation and propagate it to every child process.
    """
    if raw_value is None:
        return None
    stripped = raw_value.strip()
    ...
    if not cleaned:
        raise ValueError(
            f"{RUN_ID_ENV} must contain at least one ASCII letter or digit"
        )
    return cleaned
